...
import ast, base64, json, array, operator, re
class receive:
    ...
    def testSession(self, r):
        try:
            ...
            raise kahootError.kahootError('receive.testSession error, r: '+str(r)) from e
    ...
    def solveChallenge(self, kahoot_raw_session):
        ...
        kahootSession = array.array('B', bytes_list).tobytes().decode("ASCII")
    ...
    def computeChallenge(self, chalString):
        try:
            encoded_message, offset_expression = self.extractChallengeValues(chalString)
            offset = self.evaluateOffset(offset_expression)
        except Exception as e:
            if self.variables.debugLevel >= 2:
                print("failed to compute challenge locally:", e)
            return False

        decoded_chars = []
        for i, char in enumerate(encoded_message):
            decoded_value = (((ord(char) * i) + offset) % 77) + 48
            decoded_chars.append(chr(decoded_value))
        return ''.join(decoded_chars)

    def normalizeChallenge(self, chalString):
        challenge = str(chalString)
        challenge = re.sub(r'[\u2000-\u200f\u2028-\u202f\u205f\u2060\u3000\ufeff]', ' ', challenge)
        challenge = re.sub(r'\s+', ' ', challenge)
        return challenge.strip()

    def extractChallengeValues(self, chalString):
        challenge = self.normalizeChallenge(chalString)
        message_match = re.search(r"decode(?:\.call\(this,\s*|\()\s*(['\"])(.*?)\1\s*\)?", challenge)
        offset_match = re.search(r"var\s+offset\s*=\s*([^;]+);", challenge)

        if not message_match or not offset_match:
            raise ValueError("challenge format not recognized")

        return message_match.group(2), offset_match.group(1)

    def evaluateOffset(self, expression):
        ...
        if not isinstance(value, (int, float)):
            raise ValueError("offset is not numeric")
        return int(value)
